        ...
        return {
            ...
            "launch_summary": "\n".join(
                [
                    ...
                    "保底巡检也不能纯观察；若现场有风险，优先尝试受支持治理动作；若现场健康，也要切到更高价值且最小扰动的推进修改。",
            ...
            "execution_checklist": "\n".join(
                [
                    ...
                    "5. 本轮至少完成 1 项推进性修改；推进性修改默认指代码修复、schedule/dispatch 调整、helper 派发或恢复、发布动作、缺陷路由、版本执行约束调整中的至少一项。单纯写 history / logs / 今日日记 / 观察摘要都不算。",
                    "6. 先判断当前版本引用和当前活跃版本文件是否要求暂停、治理调整或仅观察；若是，默认不补新主线，只报告现场并保持暂停。",
                    "7. 本轮结束前，必须对当前 active 版本的每个需求点逐项更新：`状态 / 进度评估 / 预计完成时间 / 是否超时`。",
                    f"8. 若某个需求点超过上一轮承诺的预计完成时间，且本轮没有先重设 ETA，必须补写 `{SCHEDULE_VERSION_AAR_HINT}`，不能只在 history 里轻描淡写带过。",
                    f"9. 若当天学习任务尚未收口，必须给各小伙伴指派明确学习任务，并要求每个小伙伴把自己的学习报告写到 `{SCHEDULE_DAILY_LEARNING_REPORT_HINT}`；PM 不得代写空壳学习报告。",
                    "10. 检查 `/healthz`、`/api/status`、`/api/schedules`、`/api/runtime-upgrade/status`；必要时再看 `assignment graph / status-detail / run.json / events.log`。",
                    "11. 若识别到 live 风险，本轮第一优先动作默认就是尝试降低该风险；不得只把风险冻结、继续观察或原样滚到下一轮。",
                    "12. 即使主线健康、future/ready 出口存在且没有 `0 running + ready pileup` 假健康，本轮也不能只输出最小检查报告；要在不打断当前 live running 的前提下完成 1 项最小推进性修改。",
                    "13. 先记录 `root_sync_state / ahead_count / dirty_tracked_count / untracked_count / push_block_reason / next_push_batch`；若命中 dirty/ahead/异常治理现场，先处理 release boundary。",
                    ...
                    "16. 只有主链断了或当前版本引用/当前活跃版本文件明确允许时，才补新的 [持续迭代] workflow 入口；是否派发或恢复小伙伴，也要按版本文件里的每轮必查项判断。",
                    f"17. 当天的版本推进、后移、ETA 重估和后续版本排期判断先写 `{SCHEDULE_VERSION_HISTORY_HINT}`；只有主判断变化时，才更新 `pm/PM当前版本计划.md` 的当前状态快照。",
                    "18. 若发现高杠杆新功能或低维护价值重构项，先记录并明确它进入哪个后续版本，不要借巡检窗口把当前版本加胖。",
                    "19. 更细现场写入 `logs/runs/*.md` 或今日日记，不要把主计划正文写成长流水。",
                    "20. 更新 `.codex/memory/...` 时，在 `next` 明确写出下一次主线/保底触发时间；记忆库每一轮都要更新。",
                ]
            ).strip(),
            "done_definition": "\n".join(
                [
                    ...
                    "5. 若当前窗口允许推进，prod 至少保留一条未来可执行的 workflow 主线入口。",
                    "6. 每个 active 需求点都已有最新的状态 / 进度 / ETA 判断；若本轮存在超时需求，已补写对应 AAR 或明确写清不触发原因。",
                    f"7. 若当天学习任务已执行，相关学习报告已经写到 `{SCHEDULE_DAILY_LEARNING_REPORT_HINT}`，且不是 PM 代写空壳。",
                    "8. 本次巡检结论和证据可追溯。",
                    "9. 若发现上一轮遗留的 dirty/ahead 历史问题或其他工作区异常，本轮已经优先处理这批治理问题，或明确写清阻塞原因。",
                ]
            ).strip(),
        }
        ...
        return {
            ...
            "execution_checklist": "\n".join(
                [
                    ...
                    "3. 先对照上一轮结果和最近版本记录；若本轮计划与上一轮主产出实质一致，必须改选更高价值切片。",
                    "4. 本轮必须明确版本究竟推进了哪一项：`工程质量探测 / bug 探测 / 当前需求开发 / 发布推进`；若只是复述上一轮，就视为无效轮次。",
                    "5. 本轮至少完成 1 项推进性修改；推进性修改默认指代码修复、schedule/dispatch 调整、helper 派发或恢复、发布动作、缺陷路由、版本执行约束调整中的至少一项。单纯写 history / logs / 今日日记 / 观察摘要都不算。",
                    "6. 先确认当前窗口属于继续推进、异常治理还是治理调整；若当前版本引用或当前活跃版本文件写明暂停或仅观察，则不要自动扩面。",
                    "7. 再检查 `/healthz`、`/api/status`、`/api/schedules`、`/api/runtime-upgrade/status`；必要时再看 `status-detail / run.json / events.log`。",
                    "8. 若识别到 live 风险，本轮第一优先动作默认就是尝试降低该风险；不得只把风险冻结、继续观察或原样滚到下一轮。",
                    "9. 先记录 `root_sync_state / ahead_count / dirty_tracked_count / untracked_count / push_block_reason / next_push_batch`；若命中 dirty/ahead/异常治理现场，先处理 release boundary。",
                    "10. 命中工作区问题时，不能停在“等待问题被解决”。只要属于受支持动作范围，你必须主动治理收口；只有确实超出支持范围或继续动作风险更大时，才允许记为 blocked。",
                    f"11. 正式升级申请改由 `prod` supervisor 托管的 idle watcher 周期检查并发起，当前主线节点不要自己调用 `/api/runtime-upgrade/apply`。{SCHEDULE_SELF_UPGRADE_HINT}",
                    "12. 按 `质量 / 效率 / 工作区小伙伴维护 = 4 / 4 / 2` 判断重点；若 live 真相显示另一条线更高价值，主动重排优先级。",
                    "13. 只有 helper workspace 真异常时，才把工作区可用性抬成最高优先级。",
                    "14. 本轮结束前，必须对当前 active 版本的每个需求点逐项更新：`状态 / 进度评估 / 预计完成时间 / 是否超时`。",
                    f"15. 若某个需求点超过上一轮承诺的预计完成时间，且本轮没有先重设 ETA，必须补写 `{SCHEDULE_VERSION_AAR_HINT}`，不能只在 history 里轻描淡写带过。",
                    "16. 每轮都要检查是否需要给小伙伴创建、续挂、恢复或调整任务；这属于 PM 主线每轮必查项，不属于每日任务。",
                    f"17. 若当天学习任务尚未收口，必须给各小伙伴指派明确学习任务，并要求每个小伙伴把自己的学习报告写到 `{SCHEDULE_DAILY_LEARNING_REPORT_HINT}`；PM 不得代写空壳学习报告。",
                    f"18. 当天的版本推进、后移、ETA 重估和后续版本排期判断先写 `{SCHEDULE_VERSION_HISTORY_HINT}`；只有主判断变化时，才更新 `pm/PM当前版本计划.md` 的当前状态快照。",
                    "19. 若发现高杠杆新功能或低维护价值重构项，先记录并明确它进入哪个后续版本，不要继续把当前版本加胖。",
                    "20. 更细现场写入 `logs/runs/*.md` 或今日日记。",
                    "21. 更新 `.codex/memory/...` 时，在 `next` 明确写出下一次主线/保底触发时间；记忆库每一轮都要更新。",
                ]
            ).strip(),
            "done_definition": "\n".join(
                [
                    ...
                    "5. 本轮已经完成至少 1 项推进性修改；单纯写 history / logs / 今日日记 / 观察摘要不算达标。",
                    f"6. 当天的版本推进、后移、ETA 重估和后续版本排期判断已写入 `{SCHEDULE_VERSION_HISTORY_HINT}`；每个 active 需求点都有最新的状态 / 进度 / ETA 判断。",
                    f"7. 若本轮存在超时需求，已经补写 `{SCHEDULE_VERSION_AAR_HINT}`，或明确写清为什么本轮先重设 ETA 不触发 AAR。",
                    f"8. 若当天学习任务已执行，相关学习报告已经写到 `{SCHEDULE_DAILY_LEARNING_REPORT_HINT}`，且不是 PM 代写空壳。",
                    "9. 若命中工作区异常、发布边界异常或 helper 异常，本轮已经主动执行受支持的治理动作，或明确写清为什么这轮只能 blocked；不接受只写“等待问题被解决”。",
                    "10. 若本轮存在已验证代码改动，本轮结束前已经完成当前工作区 `commit / push / 根仓同步`，或明确写清阻塞原因。",
                    "11. 若当前窗口不是暂停/治理调整，本轮结束时至少还保留一个后续出口；若当前窗口是暂停/治理调整，则不得误续挂新的主线推进任务。",
                ]
            ).strip(),
        }
    return {}
