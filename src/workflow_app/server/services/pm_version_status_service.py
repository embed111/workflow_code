...
_LANE_RE = re.compile(r"当前最高价值泳道(?:为|仍为|继续保持|已切到|已更新为)?\s*`([^`]+)`")
_LIFECYCLE_STAGE_RE = re.compile(r"生命周期阶段(?:为|仍为|继续保持|已切到|已更新为)?\s*`([^`]+)`")
_BASELINE_RE = re.compile(
    r"(?:baseline=|baseline\s+(?:继续沿用|为|仍为|已切到|已更新为|已对齐为|已追到\s*live))\s*`([^`]+)`",
    re.IGNORECASE,
)
