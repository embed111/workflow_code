    ...
    return {
        ...
        "launch_summary": "\n".join(
            [
                ...
                "禁止把本轮做成上一轮的改写版；你必须先识别上一轮已经做过什么，再重新判断当前最高执行价值内容。",
        ...
        "execution_checklist": "\n".join(
            [
                ...
                "4. 先对照“上一轮结果”和最近版本记录，明确哪些动作已经做过、哪些事项只是重复复述；如果你准备做的内容与上一轮主产出实质一致，必须改选更高价值切片。",
                "5. PM 最重要的职责之一，是时刻判断版本进度有没有真的推进；本轮至少要明确回答：这轮让版本在工程质量探测、bug 探测、需求开发、发布推进中的哪一项前进了什么。若答案只是复述上一轮，就视为无效轮次。",
                "6. 按 `质量 / 效率 / 工作区小伙伴维护 = 4 / 4 / 2` 判断本轮关注重点，但不要太死板；若 live 真相显示另一条线更高价值，你必须主动重排优先级，而不是机械重复上一轮泳道。",
                "7. 当前默认优先产出应落在 `工程质量探测 / bug 探测 / 当前需求开发` 三类之一；只有 helper workspace 真异常、drift、creating 或无法派发时，才把工作区可用性重新抬成最高优先级。",
                ...
                "10. 再检查 `/healthz`、`/api/status`、`/api/schedules`、`/api/runtime-upgrade/status`；必要时再看 `status-detail / run.json / events.log`。当前 shell 是 PowerShell：不要使用 bash heredoc，不要把 `scripts/*.ps1` 这类通配路径直接交给 `rg`，也不要手工猜 run_id。",
                f"11. 正式升级申请改由 `prod` supervisor 托管的 idle watcher 周期检查并发起，当前主线节点不要自己调用 `/api/runtime-upgrade/apply`。{ASSIGNMENT_SELF_UPGRADE_HINT}",
                f"12. 每轮都要检查是否需要给 {ASSIGNMENT_SELF_ITERATION_TEAMMATES_TEXT} 创建、续挂、恢复或调整任务；这属于 PM 主线每轮必查项，不属于每日任务。",
                f"13. 当天的版本推进、后移和后续版本排期判断先写 `{ASSIGNMENT_SELF_ITERATION_VERSION_HISTORY_HINT}`；只有本轮主判断发生变化时，才更新 `{version_plan_path}` 的当前状态快照，或对应版本文件的具体排期正文。",
                "14. 若识别到新的高杠杆功能或低维护价值重构项，先记入版本记录，并明确它进入 `V2 / V3 / V4 / backlog` 的哪一处，不要继续把当前版本加胖。",
                "15. 更新 `.codex/memory/...` 时，在 `next` 明确写出下一次主线/保底触发时间；记忆库每一轮都要更新，不能因为当天每日任务已完成就跳过。",
            ]
        ).strip(),
        "done_definition": "\n".join(
            [
                ...
                "3. 本轮执行内容不能与上一轮主内容实质一致；若沿同一事项继续推进，必须新增新的证据、缺陷、实现、决策或发布动作，不接受只换措辞重复输出。",
                "4. 本轮明确记录了当前泳道、生命周期阶段，以及版本是否真的发生推进；若没有推进，必须写清卡点和下一轮如何改变动作。",
                ...
                "8. 若本轮存在已验证代码改动，本轮结束前已经完成当前工作区 `commit / push / 根仓同步`，或明确写清收口阻塞原因。",
                "9. 若当前窗口不是暂停/治理调整，本轮结束时至少还保留一个后续出口（ready / future / 明确的下一次唤醒）；若当前窗口是暂停/治理调整，则不得误续挂新的主线推进任务。",
            ]
        ),
        "priority": priority,
    ...
    return {
        ...
        "launch_summary": "\n".join(
            [
                ...
                "保底巡检也不能重复复述上一轮；若现场没有变化，要主动判断是否该把注意力切到真正能推进版本的另一条线。",
                f"先读 PM 治理入口：{ASSIGNMENT_PM_GOVERNANCE_README_PATH}",
                f"必读：{ASSIGNMENT_SELF_ITERATION_MASTER_PLAN_PATH} / {version_plan_path} / `{version_plan_path}` 中 `active_version_file` 指向的版本文件 / {ASSIGNMENT_SELF_ITERATION_DAILY_TASK_PATH} / {wake_requirement_path}",
                (
                    f"今日例行任务是否已完成，看 `{ASSIGNMENT_SELF_ITERATION_DAILY_HISTORY_HINT}`；"
        ...
        "execution_checklist": "\n".join(
            [
                ...
                f"9. 只做受支持动作：基于本机 `../workflow_code` 的 non-destructive 收口、developer workspace bootstrap/refresh、helper stale `creating` / schedule / supervisor / runtime-upgrade 恢复。不要主动 `fetch/pull origin` 或拉 GitHub。",
                f"10. 正式升级申请改由 `prod` supervisor 托管的 idle watcher 周期检查并发起，当前巡检节点不要自己调用 `/api/runtime-upgrade/apply`。{ASSIGNMENT_SELF_UPGRADE_HINT}",
                f"11. 只有主链断了，或当前版本引用/当前活跃版本文件明确要求补链/兜底时，才补新的 [持续迭代] workflow 入口；是否派发或恢复小伙伴，也要按版本文件里的每轮必查项判断。",
                f"12. 当天的版本推进、后移和后续版本排期判断先写 `{ASSIGNMENT_SELF_ITERATION_VERSION_HISTORY_HINT}`；只有本轮主判断发生变化时，才更新 `{version_plan_path}` 的当前状态快照。",
