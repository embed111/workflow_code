        ...
        payload = {
            ...
            "launch_summary": "\n".join(
                [
                    ...
                    "保底巡检也不能重复复述上一轮；若现场没有实质变化，要主动切到更高价值的探测、补链或开发方向。",
            ...
            "execution_checklist": "\n".join(
                [
                    ...
                    "4. 先对照上一轮结果和最近版本记录，判断这轮如果继续做同样的事，会不会只是重复消耗 token；若答案是会，就必须切到更高价值的巡检、探测或开发事项。",
                    "5. PM 最重要的职责之一，是时刻判断版本进度有没有真的推进；本轮至少要明确回答：当前版本在工程质量探测、bug 探测、需求开发、发布推进中的哪一项发生了新进展。",
                    ...
                    f"10. 正式升级申请改由 `prod` supervisor 托管的 idle watcher 周期检查并发起，当前巡检节点不要自己调用 `/api/runtime-upgrade/apply`。{SCHEDULE_SELF_UPGRADE_HINT}",
                    "11. 只有主链断了，或当前版本引用/当前活跃版本文件明确要求补链/兜底时，才补新的 [持续迭代] workflow 入口；是否派发或恢复小伙伴，也要按版本文件里的每轮必查项判断。",
                    f"12. 当天的版本推进、后移和后续版本排期判断先写 `{SCHEDULE_PM_VERSION_HISTORY_HINT}`；只有主判断变化时，才更新 `{SCHEDULE_PM_VERSION_PLAN_PATH}` 的当前状态快照。",
                    "13. 若发现高杠杆新功能或低维护价值重构项，先记录并明确它进入哪个后续版本，不要借巡检窗口把当前版本加胖。",
                    "14. 更新 `.codex/memory/...` 时，在 `next` 明确写出下一次主线/保底触发时间，并输出本次巡检结论与下一步建议；记忆库每一轮都要更新。",
                ]
            ).strip(),
            "done_definition": "\n".join(
                [
                    ...
                    "8. 若命中 7x24 异常治理现场，本轮已经执行受支持的治理收口动作，或明确写清为什么仍然 blocked。",
                ]
            ).strip(),
            "priority": "P1",
