...
def main() -> int:
    ...
    root = workspace_root / ".test" / "runtime-manual-recovery-thread-mode"
    if root.exists():
    ...
    graph = ws.create_assignment_graph(
        cfg,
        {
            "graph_name": "manual-recovery-thread-mode",
            "source_workflow": "test",
            "summary": "verify pm-manual-recovery non daemon",
            "review_mode": "none",
            "external_request_id": "manual-recovery-thread-mode-v1",
            "operator": "test",
        },
    )
    ...
    ws.create_assignment_node(
        ...
        ticket_id,
        {
            "node_name": "manual recovery node",
            "assigned_agent_id": "workflow",
            "priority": "P1",
            "node_goal": "verify manual recovery thread mode",
            "expected_artifact": "manual-recovery-thread-mode.md",
            "delivery_mode": "none",
            "operator": "test",
        },
    ...
    marker_path = root / "manual-recovery-marker.txt"
    child_code = f"""
import json
import sys
import time
