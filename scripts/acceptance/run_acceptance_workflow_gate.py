...
def run_runtime_upgrade_self_exclusion_probe(repo_root: Path) -> tuple[bool, dict[str, object]]:
    probe = (repo_root / "scripts" / "acceptance" / "verify_runtime_upgrade_self_exclusion.py").resolve()
    proc = subprocess.run(
        [sys.executable, str(probe)],
        cwd=str(repo_root),
        capture_output=True,
...
def run_assignment_self_upgrade_loopback_probe(repo_root: Path) -> tuple[bool, dict[str, object]]:
    probe = (repo_root / "scripts" / "acceptance" / "verify_assignment_self_upgrade_loopback.py").resolve()
    proc = subprocess.run(
        [sys.executable, str(probe)],
        cwd=str(repo_root),
        capture_output=True,
...
def run_assignment_workspace_memory_writeback_probe(repo_root: Path) -> tuple[bool, dict[str, object]]:
    probe = (repo_root / "scripts" / "acceptance" / "verify_assignment_workspace_memory_writeback.py").resolve()
    proc = subprocess.run(
        [sys.executable, str(probe)],
        cwd=str(repo_root),
        capture_output=True,
...
def run_schedule_runtime_status_persist_repair_probe(repo_root: Path) -> tuple[bool, dict[str, object]]:
    probe = (repo_root / "scripts" / "acceptance" / "verify_schedule_runtime_status_persist_repair.py").resolve()
    proc = subprocess.run(
        [sys.executable, str(probe)],
        cwd=str(repo_root),
        capture_output=True,
...
def main() -> int:
    ...
    try:
        ...
        runtime_upgrade_self_exclusion_ok, runtime_upgrade_self_exclusion_detail = run_runtime_upgrade_self_exclusion_probe(repo_root)
        results.append(
            (
                "runtime_upgrade_self_exclusion",
                runtime_upgrade_self_exclusion_ok,
                runtime_upgrade_self_exclusion_detail,
            )
        )
        if not runtime_upgrade_self_exclusion_ok:
            errors.append("runtime upgrade self exclusion probe failed")
        self_upgrade_loopback_ok, self_upgrade_loopback_detail = run_assignment_self_upgrade_loopback_probe(repo_root)
        results.append(
            (
                "assignment_self_upgrade_loopback",
                self_upgrade_loopback_ok,
                self_upgrade_loopback_detail,
            )
        )
        ...
        results.append(
            (
                "runtime_process_instance_fallback",
                runtime_process_instance_ok,
                runtime_process_instance_detail,
            )
        )
        if not runtime_process_instance_ok:
            errors.append("runtime process instance fallback probe failed")
        ...
        memory_writeback_ok, memory_writeback_detail = run_assignment_workspace_memory_writeback_probe(repo_root)
        results.append(
            (
                "assignment_workspace_memory_writeback",
                memory_writeback_ok,
                memory_writeback_detail,
            )
        )
        ...
        schedule_runtime_persist_repair_ok, schedule_runtime_persist_repair_detail = run_schedule_runtime_status_persist_repair_probe(repo_root)
        results.append(
            (
                "schedule_runtime_status_persist_repair",
                schedule_runtime_persist_repair_ok,
                schedule_runtime_persist_repair_detail,
            )
        )
        ...
        results.append(
            (
                "assignment_self_iteration_schedule_alignment",
                self_iteration_alignment_ok,
                self_iteration_alignment_detail,
            )
        )
