...
from workflow_gate_fixture_builder import init_code_root_fixture, write_agents_fixture, write_runtime_config_fixture
from workflow_gate_probe_registry import SCRIPT_PROBE_DEFINITIONS, ScriptProbeDefinition, run_script_probe
from workflow_gate_report_writer import write_gate_acceptance_report
...
def append_script_probe_result(
    repo_root: Path,
    results: list[tuple[str, bool, dict]],
    errors: list[str],
    definition: ScriptProbeDefinition,
) -> None:
    probe_ok, probe_detail = run_script_probe(repo_root, definition)
    results.append((definition.key, probe_ok, probe_detail))
    if not probe_ok:
        errors.append(definition.failure_message)
...
def main() -> int:
    ...
    try:
        ...
        for definition in SCRIPT_PROBE_DEFINITIONS:
            append_script_probe_result(repo_root, results, errors, definition)
